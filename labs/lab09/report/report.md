...
Использую режим «Быстрый просмотр» для предпросмотра содержимого файла README.md на второй панели (рис. -@fig:006).
...
Открываю файл README.md для редактирования с помощью клавиши F4 (рис. -@fig:009).
...
Копирую файл file.old в созданный каталог testdir с помощью F5 (рис. -@fig:012).
...
Получаю результат поиска — найден файл program.cpp в каталоге /home/dbignatenko/testdir (рис. -@fig:014).
...
Создаю текстовый файл text.txt и открываю его в редакторе mc. Вставляю в файл текст Lorem Ipsum (рис. -@fig:019).
...
Выделяю фрагмент текста (F3) и копирую его на новую строку (F5). Фрагмент «It is a long» скопирован (рис. -@fig:022).

![Копирование фрагмента текста](image/22.png){#fig:022 width=70%}

Выделяю фрагмент текста и перемещаю его на новую строку (F6). Добавляю текст «It is a long time.3» (рис. -@fig:023).
...
Перехожу в начало файла (Ctrl-Home) и добавляю текст «start» (рис. -@fig:026).

![Переход в начало файла и добавление текста](image/26.png){#fig:026 width=70%}

### Работа с файлом исходного кода

Открываю файл program.cpp с исходным кодом на C++. Подсветка синтаксиса включена — ключевые слова, типы данных и строки выделены разными цветами (рис. -@fig:027).

![Файл program.cpp с подсветкой синтаксиса](image/27.png){#fig:027 width=70%}

Просматриваю файл program.cpp с подсветкой синтаксиса. Видно выделение директивы #include, ключевых слов int, return, строкового литерала (рис. -@fig:028).
