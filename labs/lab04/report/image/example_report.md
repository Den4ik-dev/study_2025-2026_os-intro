...
Открываю файл report.md в текстовом редакторе (не использовал команду gedit, просто открыл файл через графический интерфейс). Заполняю YAML-шапку документа: указываю название лабораторной работы (title), дисциплину (subtitle), автора (author), а также настройки форматирования для PDF-вывода (рис. -@fig:001).

![Заполнение YAML-шапки документа](image/1.png){#fig:001 width=70%}

## Написание цели, задания и теоретического введения

Формирую основные разделы отчёта: цель работы, задание и теоретическое введение. В теоретическом введении описываю основы работы с системами контроля версий и добавляю таблицу с основными командами git (рис. -@fig:002).
...
Выполняю компиляцию отчёта с помощью команды make. Pandoc обрабатывает файл report.md и создаёт файлы report.pdf и report.docx (рис. -@fig:006).
...
Проверяю содержимое каталога с помощью команды ls. В каталоге присутствуют все необходимые файлы: report.md (исходный файл), report.pdf и report.docx (скомпилированные версии) (рис. -@fig:007).
