...
Chezmoi — утилита для управления файлами конфигурации (dotfiles) домашнего каталога пользователя. Состояние файлов конфигурации сохраняется в каталоге `~/.local/share/chezmoi`, который является клоном репозитория dotfiles.
...
```bash
pass init ignatenko.de2017@yandex.ru
```
...
Репозиторий успешно создан и готов к использованию (рис. [-@fig:005]).
...
```bash
pass git init
pass git remote add origin git@github.com:Den4ik-dev/password-store.git
```
...
Проверяю репозиторий на GitHub — зашифрованный файл mysite.gpg успешно загружен (рис. [-@fig:008]).
...
Инициализирую chezmoi с созданным репозиторием dotfiles, просматриваю изменения и применяю их (рис. [-@fig:014]).

```bash
chezmoi init git@github.com:Den4ik-dev/dotfiles.git
chezmoi diff
chezmoi apply -v
```
...
```toml
[data]
    email = "ignatenko.de2017@yandex.ru"
