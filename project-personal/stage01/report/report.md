...
Устанавливаю необходимые пакеты через менеджер пакетов dnf: Git, Go (golang) и Node.js с npm (рис. -@fig:001).
...
Перехожу на страницу шаблона HugoBlox/hugo-theme-academic-cv на GitHub и нажимаю «Use this template». Создаю новый репозиторий с именем `den4ik-dev.github.io` — это важно, так как имя репозитория должно соответствовать формату `USERNAME.github.io` для корректной работы GitHub Pages (рис. -@fig:004).
...
Клонирую созданный репозиторий на локальную машину с флагом `--recurse-submodules` для загрузки всех подмодулей темы (рис. -@fig:006).

```bash
git clone --recurse-submodules git@github.com:Den4ik-dev/den4ik-dev.github.io.git
cd den4ik-dev.github.io
```
...
```yaml
baseURL: 'https://den4ik-dev.github.io/'
```
...
Открываю сайт по публичному адресу `https://den4ik-dev.github.io/`. Сайт отображается корректно — загружаются стили, шрифты и демонстрационный контент шаблона (рис. -@fig:014).
...
| Команда | Описание |
|---------|----------|
| `sudo dnf install git golang nodejs npm` | Установка необходимого ПО |
| `hugo version` | Проверка версии Hugo |
| `git clone --recurse-submodules <url>` | Клонирование репозитория с подмодулями |
...
В ходе выполнения первого этапа индивидуального проекта установил необходимое программное обеспечение (Git, Go, Node.js, Hugo Extended), создал репозиторий на GitHub на основе шаблона Hugo Academic Theme, настроил baseURL и GitHub Pages для публикации сайта. Заготовка персонального сайта научного работника успешно размещена и доступна по адресу `https://den4ik-dev.github.io/`.
